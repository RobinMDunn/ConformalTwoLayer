...
#'
#' @examples
sup_pool_cdfs_split <- function(xy_data, alpha, k_val, k_model_fit, X_new,
                                Y_new = NULL) {
  ...
  if(k_val >= 2) {
    X_model_fit <- unlist(xy_data$X[k_model_fit])
    Y_model_fit <- unlist(xy_data$Y[k_model_fit])
  } else if(k_val == 1) {
  ...
  X_resid_fit <- xy_data$X[k_resid_fit]
  Y_resid_fit <- xy_data$Y[k_resid_fit]
  resid_fit <-
    lapply(1:length(k_resid_fit),
           FUN = function(i) abs(Y_resid_fit[[i]] -
                                   predict(model_fit,
                                           newdata = data.frame(X_model_fit = X_resid_fit[[i]]))))
  ...
  predict_Y <- predict(model_fit, newdata = data.frame(X_model_fit = X_new))
