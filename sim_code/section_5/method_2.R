...
for(row in 1:nrow(results)){
  ...
  # Keep coverage and length as NA unless n_1 > 1/alpha - 1 and k >= 2
  if(n_val > 1 / alpha - 1 & k_val >= 2) {
    ...
    # Run n_sim shrinkage simulations
    for(sim in 1:n_sim) {
      ...
      unsup_shrinkage_results <- unsup_method_shrinkage(Y = Y, alpha = alpha,
                                                        Y_new = Y_new)

      covered[sim] <- unsup_shrinkage_results$covered

      pred_int_size[sim] <- unsup_shrinkage_results$pred_int_size
    }
