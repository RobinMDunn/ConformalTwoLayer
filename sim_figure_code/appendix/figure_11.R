# Coverage plots for supervised methods on normal data
...
cov_small_k <- results %>%
  ...
  facet_grid(Params ~ n, labeller = label_parsed) +
  ...
  labs(x = "Number of Groups (k)",
       y = "Coverage",
       title = expression("Supervised Prediction. Coverage by Method."~
...
ggsave(plot = cov_small_k,
       filename = "sim_figures/appendix/sup_cov_small_k.pdf",
       width = 8.5, height = 4.5)

ggsave(plot = cov_large_k,
       filename = "sim_figures/appendix/sup_cov_large_k.pdf",
       width = 8.5, height = 4.5)
