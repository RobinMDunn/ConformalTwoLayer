# Size plots for unsupervised methods on normal data
...
size_small_k <- results %>%
  ...
  mutate(Method = factor(
    ...
    labels = c("0. Double Conformal",
               ...
               "2. Subsample Once",
               "3. Repeated Subsample"))) %>%
  mutate(n = factor(n, levels = c(40, 100, 1000),
  ...
  labs(x = "Number of Groups (k)",
       ...
       title = "Unsupervised Prediction. Interval Length by Method, Smaller k Values.") +
...
size_large_k <- results %>%
  ...
  labs(x = "Number of Groups (k)",
       y = "Average Prediction Interval Length",
       title = "Unsupervised Prediction. Interval Length by Method, Larger k Values.") +
...
ggsave(plot = size_small_k,
       filename = "sim_figures/appendix/unsup_size_small_k.pdf",
       width = 9.5, height = 3.5)

ggsave(plot = size_large_k,
       filename = "sim_figures/appendix/unsup_size_large_k.pdf",
       width = 9.5, height = 3.5)
